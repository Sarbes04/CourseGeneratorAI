...
function EnrolledCourseInfo({ course, viewCourse=true }) {
  ...
  const GenerateCourseContent = async () => {
    ...
    setLoading(true);
    try {
      const result = await axiosInstance.post("/api/generate-course/generate", {
    ...
    <div className="flex-row-reverse md:flex gap-5 justify-between p-5 rounded-2xl shadow">
      <div className="flex flex-col gap-3">
        ...
        {!viewCourse ? (
          ...
          <button
            //onClick={() => navigate(`workspace/view/${course?.cid}`)}
            className="w-full flex items-center gap-2 justify-center px-4 py-2 rounded-md text-white bg-green-600 hover:bg-green-700 transition"
          >
            <PlayCircle />
            Continue Learning
          </button>
